...
function gridDirective($parse: ng.IParseService, config: IMainConfig, common: ICommon) {
    function compile(cElement: ng.IAugmentedJQuery, cAttrs: IDateRangeDirectiveAttrs) {
        ...
        //Show time
        if (angular.isDefined(cAttrs.showTime)) {
            $sd.attr('show-time', '');
            $ed.attr('show-time', '');
        }
        //#endregion
...
//#region Injections
gridDirective.$inject = ['$parse', 'Config', 'Common'];
//#endregion
//#endregion

//#region Register
var module: ng.IModule = angular.module('rota.directives.rtdaterange', []);
module.directive('rtDateRange', gridDirective);
//#endregion
