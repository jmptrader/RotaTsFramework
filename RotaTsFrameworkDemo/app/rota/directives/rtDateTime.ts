 ...
 * Datetime attributes
 */
interface IDateTimeDirectiveAttrs extends ng.IAttributes {
    showTime: boolean;
}
/**
 * Datetime scope
...
function dateTimePickerDirective(config: IMainConfig, common: ICommon) {
    function compile(cElement: ng.IAugmentedJQuery, cAttrs: IDateTimeDirectiveAttrs) {
        ...
        return (scope: IDatetimeScope, element: ng.IAugmentedJQuery, attrs: IDateTimeDirectiveAttrs, modelCtrl: ng.INgModelController): void => {
            ...
            scope.onTimeSet = (newDate: Date, oldDate: Date) => {
                scope.openIt = false;
            };
        }
    }
    ...
    const directive = <ng.IDirective>{
        ...
        template: '<span class="rt-datetime" uib-dropdown is-open="openIt">' +
        ...
        '<datetimepicker ng-model=ngModel data-on-set-time=onTimeSet()/></ul></span>'
    };
    return directive;
    //#endregion
}
...
//#region Register
const module: ng.IModule = angular.module('rota.directives.rtdatetime', []);
module.directive('rtDatetime', dateTimePickerDirective).run([
